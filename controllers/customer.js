    ...
    if (data && data.status == 1 && data.customer_orders.length > 0) {
        ...
        for (var key = 0, size = customer_orders.length; key < size; key++) {
            html += '<tr ><td class="mdl-data-table__cell--non-numeric">' +
                ...
                '<a class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--icon modal-trigger"   onclick="return ViewCustomerOrderInfo(\'' + customer_orders[key].customer_order_id + '\' )">' +
