...
function PaymentType() {
  ...
  this.getAll = function (res) {
    var output = {}, query = 'SELECT * FROM payment_type';
    ...
    connection.acquire(function (err, con) {
      ...
      con.query(query, function (err, result) {
        ...
        if(err){
          ...
          if(result.length > 0){
            output = {status: 1, payment_types: result};
        }
        else{
            output = {status: 0, message:'No payment type found'};
        }
        res.json(output);
        }
      });
  ...
  this.getOne = function (id, res) {
    var output = {}, query = 'SELECT * FROM payment_type WHERE payment_type_id = ?';
    ...
    connection.acquire(function (err, con) {
      ...
      con.query(query, id, function (err, result) {
        ...
        if(result.length > 0){
          output = {status: 1, payment_type: result[0]};
      }
      else{
          output = {status: 0, message:'No such payment type found'};
      }
      res.json(output);
      }
      });
  ...
  this.update = function (paymentTypeObj, res) {
    ...
    var feedback, payment_type_name = paymentTypeObj.payment_type_name, payment_type_id = paymentTypeObj.payment_type_id;
